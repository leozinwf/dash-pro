// src/app/(dashboard)/projetos/[id]/editar/page.tsx
"use client"
...
export default function EditarProjetoPage() {
  ...
  // Busca os dados atuais do projeto
  useEffect(() => {
    async function fetchProject() {
      ...
      if (error) {
        ...
        setEnvLocalFile(data.env_local_file || '')
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const cleanGithubRepo = githubRepo.replace('https://github.com/', '').replace('.git', '').trim()
  ...
  if (initialLoading) return <div className="p-8 text-center text-gray-500">Carregando projeto...</div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
            ...
            <div>
              ...
              <input type="text" required value={nome} onChange={(e) => setNome(e.target.value)} className="w-full px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-2"><FolderGit size={16} /> Repositório GitHub</label>
                <input type="text" value={githubRepo} onChange={(e) => setGithubRepo(e.target.value)} className="w-full px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-2"><LinkIcon size={16} /> URL do Projeto</label>
                <input type="url" value={projetoUrl} onChange={(e) => setProjetoUrl(e.target.value)} className="w-full px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="password" value={databasePassword} onChange={(e) => setDatabasePassword(e.target.value)} placeholder="••••••••" className="w-full px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" />
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-2"><AlignLeft size={16} /> Observações</label>
              <textarea value={observacoes} onChange={(e) => setObservacoes(e.target.value)} rows={3} className="w-full px-4 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition resize-none"></textarea>
            </div>
            ...
            <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
              ...
              <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium transition disabled:opacity-50">
                {loading ? 'Atualizando...' : <><Save size={20} /> Atualizar</>}
              </button>
            </div>
          </form>
        </div>
